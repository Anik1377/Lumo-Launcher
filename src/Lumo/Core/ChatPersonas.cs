...
/// chat (tone, format, focus), PLUS a face: a tiny vector character (see
/// PersonaFaces) and a color that tint the chat's avatars and mascot.
/// The registry is pure data on purpose: the test harness pins ids, glyphs and
/// prompt lengths, and the request builders treat the prompt as an opaque string.
/// </summary>
public sealed record ChatPersona(
    string Id, string Name, string Glyph, string Prompt, string Blurb,
